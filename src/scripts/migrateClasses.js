...
//const classesTableKey = 'prod-the-studio-classes-v2';
const usersTableKey = "prod-the-studio-users";
...
const finalUsers = {
  [usersTableKey]: users.map((user) => {
    const filteredBookedClasses = user.bookedClasses?.L.filter(
      (bookedClass) => {
    ...
    const newPurchasedPackages = user.purchasedPackages?.L.map(
      (purchasedPackage) => {
    ...
    console.log("newPurchasedPackages", newPurchasedPackages);

    const filteredPurchasedPackages = newPurchasedPackages?.filter(
      (purchasedPackage) => {
    ...
    console.log("filteredPurchasedPackages", filteredPurchasedPackages);
    ...
    return {
      PutRequest: {
        Item: {
          ...
          purchasedPackages: filteredPurchasedPackages
            ? { L: filteredPurchasedPackages }
            : { L: [] },
        },
      },
    };
...
for (let i = 0; i < userFiles; i++) {
  fs.writeFileSync(
    `./src/scripts/newUsers${i + 1}.json`,
    JSON.stringify({
      [usersTableKey]: finalUsers[usersTableKey].slice(
        i * batchSize,
        (i + 1) * batchSize
      ),
    })
